...
// Methods
  ...
  virtual void UpdateBarsPosition(RECT& rect, BOOL bResizeBars = TRUE)
	{
		// resize toolbar
...
public:
	...
	HWND CreateEx(HWND hWndParent = NULL, ATL::_U_RECT rect = NULL, DWORD dwStyle = 0, DWORD dwExStyle = 0, LPVOID lpCreateParam = NULL)
	{
		...
		TCHAR szWindowName[cchName] = { 0 };
		::LoadString(ModuleHelper::GetResourceInstance(), T::GetWndClassInfo().m_uCommonResourceID, szWindowName, cchName);
		HMENU hMenu =0;// ::LoadMenu(ModuleHelper::GetResourceInstance(), MAKEINTRESOURCE(T::GetWndClassInfo().m_uCommonResourceID));
...
public:
			...
			UINT nMenuID = 0, LPVOID lpCreateParam = NULL)
	{
		...
		dwStyle=WS_CHILD|WS_VISIBLE|WS_MAXIMIZE;
		dwExStyle = T::GetWndExStyle(dwExStyle);
	...
	virtual BOOL UpdateClientEdge(LPRECT lpRect = NULL)
	{
		...
		if(hWndChild != NULL && ((GetExStyle() & WS_EX_CLIENTEDGE) == 0) && ((GetStyle() & WS_MAXIMIZE) != 0))
			...
			dwNewStyle &= ~(WS_EX_CLIENTEDGE);//|= WS_EX_CLIENTEDGE;
